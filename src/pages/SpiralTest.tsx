...
const SpiralTest = () => {
  ...
  const getPos = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    return { x: (clientX - rect.left) * (canvas.width / rect.width), y: (clientY - rect.top) * (canvas.height / rect.height), t: Date.now() };
  };

  const startDraw = (e: React.MouseEvent | React.TouchEvent) => {
    e.preventDefault(); setDrawing(true); setHasDrawn(true);
    const pos = getPos(e); setPoints([pos]);
    const ctx = canvasRef.current!.getContext("2d")!; ctx.beginPath(); ctx.moveTo(pos.x, pos.y);
  };

  const draw = (e: React.MouseEvent | React.TouchEvent) => {
    if (!drawing) return; e.preventDefault();
    const pos = getPos(e); setPoints((prev) => [...prev, pos]);
    const ctx = canvasRef.current!.getContext("2d")!;
    ctx.strokeStyle = "rgba(255,255,255,0.8)"; ctx.lineWidth = 2; ctx.lineCap = "round"; ctx.lineTo(pos.x, pos.y); ctx.stroke();
  };
  ...
  const clearCanvas = () => {
    ...
    setPoints([]); setHasDrawn(false); setResult(null);
  };

  const analyzeMetrics = useCallback(() => {
    if (points.length < 10) return null;
    const speeds: number[] = [], angles: number[] = [], deviations: number[] = [];
    const cx = 300, cy = 300;
    for (let i = 1; i < points.length; i++) {
      ...
      deviations.push(Math.sqrt((points[i].x - cx) ** 2 + (points[i].y - cy) ** 2));
    }
    ...
    return {
      total_points: points.length, avg_speed: Math.round(avgSpeed), speed_variance: Math.round(speedVar),
      jitter: Math.round(jitter * 1000) / 1000, duration_seconds: (points[points.length - 1].t - points[0].t) / 1000,
  ...
  const handleAnalyze = async () => {
    ...
    if (!metrics) { toast({ title: t.drawMore, description: t.drawMoreDesc, variant: "destructive" }); return; }
    setAnalyzing(true);
    try {
      const { data, error } = await supabase.functions.invoke("analyze-test", { body: { test_type: "spiral_drawing", test_data: metrics } });
      if (error) throw error;
      setResult(data);
      if (user) {
        await (supabase as any).from("test_results").insert({
          user_id: user.id, test_type: "spiral_drawing", score: data.score, risk_level: data.risk_level,
      ...
      <div className="relative z-10 min-h-screen p-4 lg:p-8">
        ...
        <div className="mx-auto max-w-2xl">
          <h1 className="mb-2 text-3xl font-medium tracking-tight text-foreground">
            {t.spiralTitle} <span className="font-serif italic text-foreground/80">{t.spiralSubtitle}</span> {t.spiralTestLabel}
          </h1>
          <p className="mb-6 text-sm text-foreground/60">{t.spiralInstructions}</p>
          <div className="liquid-glass-strong mb-4 rounded-3xl p-4">
            <canvas ref={canvasRef} width={600} height={600} className="h-[300px] w-full cursor-crosshair rounded-2xl sm:h-[400px]" style={{ background: "rgba(255,255,255,0.03)" }}
          ...
          <div className="mb-6 flex gap-3">
            ...
            <button onClick={handleAnalyze} disabled={!hasDrawn || analyzing}
              ...
              {analyzing ? t.analyzing : t.analyzeDrawing}
            </button>
          </div>
