...
const TappingTest = () => {
  ...
  const analyzeMetrics = () => {
    ...
    return {
      ...
      interval_variance: Math.round(variance),
  ...
  const handleAnalyze = async () => {
    ...
    if (!metrics) {
      toast({ title: "Not enough taps", description: "Please tap more during the test", variant: "destructive" });
      return;
    }
    ...
    try {
      const { data, error } = await supabase.functions.invoke("analyze-test", {
        body: { test_type: "finger_tapping", test_data: metrics },
      });
      ...
      await supabase.from("test_results").insert({
        user_id: user!.id,
        test_type: "finger_tapping" as any,
        score: data.score,
      ...
      <div className="relative z-10 min-h-screen p-4 lg:p-8">
        ...
        <div className="mx-auto max-w-lg">
          <h1 className="mb-2 text-3xl font-medium tracking-tight text-foreground">
            Finger <span className="font-serif italic text-foreground/80">Tapping</span> Test
          </h1>
          <p className="mb-8 text-sm text-foreground/60">
            Tap the button as fast and steadily as you can for {TEST_DURATION} seconds.
          </p>
          ...
          <div className="liquid-glass-strong mb-6 flex flex-col items-center rounded-3xl p-8">
            {phase === "ready" && (
              <button
                onClick={startTest}
                className="flex h-32 w-32 items-center justify-center rounded-full bg-foreground/10 transition-transform hover:scale-110 active:scale-95"
              >
            ...
            {phase === "testing" && (
              <>
                ...
                <button
                  onClick={handleTap}
                  className="flex h-40 w-40 items-center justify-center rounded-full bg-foreground/10 transition-transform active:scale-90"
                >
                  <Hand className="h-12 w-12 text-foreground" />
                </button>
                <p className="mt-4 text-lg font-medium text-foreground">{taps.length} taps</p>
              </>
            )}
            ...
            {phase === "done" && (
              <div className="text-center">
                ...
                <p className="text-sm text-foreground/60">taps in {TEST_DURATION} seconds</p>
                <p className="text-xs text-foreground/40">
                  ({(taps.length / TEST_DURATION).toFixed(1)} taps/sec)
                </p>
              </div>
            )}
          </div>
          ...
          {phase === "done" && (
            <div className="flex gap-3">
              <button
                onClick={() => { setPhase("ready"); setTaps([]); setResult(null); }}
                className="liquid-glass flex-1 rounded-full py-3 text-sm text-foreground transition-transform hover:scale-105"
              >
                Try Again
              </button>
              <button
